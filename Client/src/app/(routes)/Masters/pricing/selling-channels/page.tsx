...
function SellingChannelsPage() {
  ...
  const { data: sellingChannelsData, isLoading: isSellingChannelsLoading, refetch: refetchSellingChannels } = useFetchSellingChannels();
  ...
  // Handle row click to log data
  const handleRowClick = (params: any) => {
    console.log('Row data:', params.row);
    
    // The event structure is different than expected
    // Simply open the edit drawer when a row is clicked
  ...
  const processedData = useMemo(() => {
    ...
    return sellingChannelsData.map((channel: SellingChannel) => ({
      ...channel,
  ...
  const tabCounts = useMemo(() => {
    if (!processedData) return { all: 0, active: 0, inactive: 0 };
    
    const all = processedData.length;
    const active = processedData.filter(item => item.is_active).length;
    const inactive = processedData.filter(item => !item.is_active).length;
      ...
      >
        {viewMode === 'list' ? (
          <CustomDataGrid
            ...
            loading={isSellingChannelsLoading || isLoadingChannel}

                ...
                return <Controller
                    ...
                    render={({ field, fieldState }) => {
                        ...
                        const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
                            ...
                            // Check if input exceeds max digits (excluding minus sign)
                            const valueToCheck = val.startsWith('-') ? val.substring(1) : val;
                            if (valueToCheck.length > maxDigits && val !== '') {
                                // Don't update if exceeds max digits
                                return;
                            }
                        ...
                        const handleBlur = (e: React.FocusEvent<HTMLInputElement>) => {
                            ...
                            // Note: We're not auto-correcting anymore, just triggering validation to show error messages
                            ...
                            <TextField
                                ...
                                onKeyDown={(e) => {
                                    ...
                                    if (['ArrowUp', 'ArrowDown', 'Enter'].includes(e.key) && 
                                        hasNumberConstraints && methods?.trigger) {
                                        setTimeout(() => methods.trigger(fieldNameValue), 0);
                                        return;
                                    }
                                    
                                    // Prevent entering more digits than allowed by the validation range
                                    if (hasNumberConstraints && e.key !== 'Backspace' && e.key !== 'Delete' && 
                                        e.key !== 'ArrowLeft' && e.key !== 'ArrowRight' && e.key !== 'Tab' && 
                                        e.key !== '-' && e.key !== '.') {
                                        
                                        const currentValue = field.value?.toString() || '';
                                        const valueToCheck = currentValue.startsWith('-') ? currentValue.substring(1) : currentValue;
                                        
                                        // Check if we're at max digits and trying to add more
                                        if (valueToCheck.length >= maxDigits && 
                                            !e.ctrlKey && // Allow copy/paste shortcuts
                                            !e.metaKey && // Allow cmd key on Mac
                                            !e.altKey) { // Allow alt combinations
                                            
                                            // Don't allow selection replacement if result would be too long

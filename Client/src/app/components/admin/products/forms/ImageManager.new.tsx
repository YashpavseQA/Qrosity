...
import { useUploadTemporaryImage, TemporaryUploadResponse } from '@/app/hooks/api/assets';
...
/**
 * Interface for the data to be sent to the server for temporary images
 */
interface TemporaryImageData {
  id: string;
  alt_text: string;
  is_default: boolean;
  sort_order: number;
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    for (const tempImage of newTempImages) {
      try {
        ...
        // Upload the image
        const response = await uploadTempImage(tempImage.file);

...
const TaxRegionForm = forwardRef<TaxRegionFormRef, TaxRegionFormProps>(({
  ...
  // Fetch countries for the dropdown - using the shared API hook
  const { data: countriesData, isLoading: isLoadingCountries } = useFetchCountries({ is_active: true });
  ...
  const countriesMap = useMemo(() => {
    ...
    if (countriesData) {
      ...
      const activeCountries = Array.isArray(countriesData) 
        ? countriesData.filter(country => country.is_active)
        : [];
                ...
                <Autocomplete
                  ...
                  getOptionLabel={(option) => {
                    ...
                    if (typeof option === 'number') {
                      ...
                      return country ? `${country.name} (${country.code})` : String(option);
                    }
